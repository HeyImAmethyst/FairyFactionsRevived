...
public class FairyFishHookEntityRenderer extends EntityRenderer<FairyFishHookEntity>
{
    ...
    public FairyFishHookEntityRenderer(EntityRendererProvider.Context p_174117_)
    {
        super(p_174117_);
    }

    public void render(FairyFishHookEntity pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight)
    {
        FairyEntity fairy = pEntity.getFairyOwner();
        ...
        if (fairy != null)
        {
            ...
            int i = fairy.getMainArm() == HumanoidArm.RIGHT ? 1 : -1;

            float f = fairy.getAttackAnim(pPartialTick);
            float f1 = Mth.sin(Mth.sqrt(f) * (float)Math.PI);
            float f2 = Mth.lerp(pPartialTick, fairy.yBodyRotO, fairy.yBodyRot) * ((float)Math.PI / 180F);
            ...
            d4 = Mth.lerp((double)pPartialTick, fairy.xo, fairy.getX()) - d1 * d2 - d0 * 0.8D;
            d5 = fairy.yo + (double)fairy.getEyeHeight() + (fairy.getY() - fairy.yo) * (double)pPartialTick - 0.45D;
            d6 = Mth.lerp((double)pPartialTick, fairy.zo, fairy.getZ()) - d0 * d2 + d1 * 0.8D;
            f3 = fairy.isCrouching() ? -0.1875F : 0.0F;

            double d9 = Mth.lerp((double)pPartialTick, pEntity.xo, pEntity.getX());
            double d10 = Mth.lerp((double)pPartialTick, pEntity.yo, pEntity.getY()) + 0.25D;
            double d8 = Mth.lerp((double)pPartialTick, pEntity.zo, pEntity.getZ());
    ...
    public ResourceLocation getTextureLocation(FairyFishHookEntity p_114703_)
    {
